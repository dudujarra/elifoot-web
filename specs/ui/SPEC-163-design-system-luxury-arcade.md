...
## Pergunta/Objetivo
Como unificar as diretrizes de design do Elifoot RPG para garantir uma estética consistente e de alta qualidade (Luxury Arcade) em toda a interface do usuário?

## Sintoma/Motivação (Por que foi feito)
Existiam dois documentos de design conflitantes (`ELIFOOT_DESIGN_SYSTEM.md` na raiz e `.agent/DESIGN.md`), causando divergências (ex: o uso da fonte 'Inter' era banido no documento principal, mas prescrito no arquivo `.agent`). A duplicidade gerava inconsistências arquitetônicas na UI.

## Método
- Exclusão do arquivo desatualizado e incorreto `.agent/DESIGN.md`.
- Formalização do `ELIFOOT_DESIGN_SYSTEM.md` como a única fonte canônica da verdade visual para todos os agentes.
...
- A paleta de cores, tipografia (sem Inter), e iconografia (sem Emojis) devem ser estritamente controladas pelas diretrizes do `ELIFOOT_DESIGN_SYSTEM.md`.
