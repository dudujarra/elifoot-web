...
const DIV_PRESTIGE: Record<number, number> = { 1: 80, 2: 55, 3: 35, 4: 15 };
const DIV_NAMES: Record<number, string> = { 1: 'Série A', 2: 'Série B', 3: 'Série C', 4: 'Série D' };
...
export interface PlayerData {
    ...
    form?: { trend: string };
    age?: number;
    _seasonsAtClub?: number;
    _retired?: boolean;
    injury?: any;
    _satisfaction?: number;
    _transferRequested?: boolean;
    _transferRequestReason?: string;
    _relegationClause?: boolean;
    _relegationClauseValue?: number;
    value?: number;
    marketValue?: number;
    position?: string;
    salary?: number;
 ...
 */
export function calcPrestige(team: TeamData): number {
    ...
    const avgOvr = team.squad && team.squad.length > 0
        ? team.squad.reduce((s: number, p: PlayerData) => s + (p.ovr || 50), 0) / team.squad.length
        : 50;
...
const PERSONALITY_AMBITION_MOD: Record<string, number> = {
 ...
 */
export function calcAmbition(player: PlayerData): number {
    const mod = PERSONALITY_AMBITION_MOD[player.personality || 'Profissional'] || 1.0;
 ...
 */
export function calcSatisfaction(player: PlayerData, team: TeamData): number {
...
export function processAmbitionWeekly(team: TeamData): AmbitionEvent[] {
    const events = [];
    ...
    for (const player of (team.squad || [])) {
        ...
        if (sat < SATISFACTION.CRISIS) {
            ...
            if (sat < SATISFACTION.REBELLION && (player.ovr || 50) >= OVR.STAR) {
                events.push({
 ...
 */
export function onRelegation(team: TeamData, fromDiv: number, toDiv: number): AmbitionEvent[] {
    const events = [];
    ...
    const sortedByOvr = [...(team.squad || [])].sort((a: PlayerData, b: PlayerData) => (b.ovr || 0) - (a.ovr || 0));
 ...
 */
export function onPromotion(team: TeamData, fromDiv: number, toDiv: number): AmbitionEvent[] {
    const events = [];
    const divName = DIV_NAMES[toDiv] || `Divisão ${toDiv}`;
 ...
 */
export interface ContextualBuyerResult {
    teamId: number;
    teamName: string;
    division: number;
    prestige: number;
...
export function findContextualBuyers(player: PlayerData, allTeams: TeamData[], sellerTeam: TeamData): ContextualBuyerResult[] {
    const ambition = calcAmbition(player);
    const playerValue = player._relegationClause
        ? (player._relegationClauseValue || 1_000_000)
        : (player.value || player.marketValue || 1_000_000);

    const candidates = (allTeams || [])
        .filter((t: TeamData) => t.id !== sellerTeam?.id)
        .filter((t: TeamData) => (t.balance || 0) > playerValue * 0.5) // pode pagar pelo menos metade
        .map((t: TeamData) => {
            const prestige = t._prestige || calcPrestige(t);
            const score = matchScore(t, player, prestige, ambition);
            return { team: t, score, prestige };
        })
        .filter((c: { team: TeamData; score: number; prestige: number }) => c.score > 20) // mínimo de compatibilidade
        .sort((a: { team: TeamData; score: number; prestige: number }, b: { team: TeamData; score: number; prestige: number }) => b.score - a.score)
        .slice(0, 3);

    return candidates.map((c: { team: TeamData; score: number; prestige: number }) => ({
        teamId: c.team.id || 0,
        teamName: c.team.name || 'Clube',
        division: c.team.division,
        prestige: c.prestige,
        matchScore: c.score,
        offerPrice: calcOffer(player, c.team, playerValue || 1_000_000),
    }));
}

function matchScore(team: TeamData, player: PlayerData, teamPrestige: number, playerAmbition: number): number {
    let score = 0;
    ...
    const posCount = (team.squad || []).filter((p: PlayerData) => p.position === player.position && !p._retired).length;
...
function calcOffer(player: PlayerData, buyerTeam: TeamData, baseValue: number): number {
