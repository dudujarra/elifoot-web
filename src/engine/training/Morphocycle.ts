...
class Morphocycle {
     ...
     */
    calculateDailyLoad(player: Player, sessionKey: string, minutesPlayed: number | null = null) {
     ...
     */
    calculateWeeklyStrain(dailyLoads: number[]) {
        if (dailyLoads.length !== 7) {
            throw new Error('dailyLoads array must contain exactly 7 days of data.');
        }

        const weeklyLoad = dailyLoads.reduce((sum: number, load: number) => sum + load, 0);
        const meanLoad = weeklyLoad / 7;

        // Calculate Standard Deviation
        const variance = dailyLoads.reduce((sum: number, load: number) => sum + Math.pow(load - meanLoad, 2), 0) / 7;
     ...
     */
    calculateACWR(acuteLoad: number, chronicLoads: number[]) {
        ...
        const avgChronic = chronicLoads.reduce((a: number, b: number) => a + b, 0) / chronicLoads.length;
        if (avgChronic === 0) {
        ...
        if (acwr < 0.8) {
            ...
            risk = 'High';
            message = 'Danger Zone. Spikes in load increase injury risk exponentially.';
        } else if (acwr >= 1.3) {
            risk = 'Caution';
     ...
     */
    applyTrainingGrowth(player: Player, sessionKey: string) {
        const deltas: Record<string, Record<string, number>> = { technical: {}, mental: {}, physical: {}, goalkeeping: {} };
    ...
    DAYS: string[];
    SESSION_TYPES: Record<string, { rpe: number, duration: number, type: string, load: number }>;
}
