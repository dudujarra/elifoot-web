...
export function calcMarketValue({ playerOvr, playerAge, playerPotential, playerContract = 26, playerForm = 0 }: MarketValueOptions): number {
...
export interface MakeOfferOptions {
    ...
    sellingWillingness?: 'forced' | 'open' | 'reluctant';
    seed?: number | null;
}
...
export function makeOffer({ playerOvr, playerAge, playerPotential, playerContract = 26, playerForm = 0, need = 'medium', sellingWillingness = 'open', seed = null }: MakeOfferOptions): OfferResult {
...
export interface TransferOffer {
    ...
    isRelegationClause: boolean;
}

export function generateRealTransferOffers(team: Team, currentWeek: number, allTeams: any[] = []): TransferOffer[] {
    ...
    team.squad.forEach((player: Player) => {
        ...
        if (player.ovr >= 65 && systemRng() < baseChance) {
            ...
            const result = makeOffer({
                ...
                playerContract: typeof player.contract?.weeksLeft === 'number' ? player.contract.weeksLeft : 26,
                playerForm: 0,
                need,
            ...
            if (player._relegationClause && player._relegationClauseValue) {
                result.offerPrice = Math.min(result.offerPrice, player._relegationClauseValue as number);
            }
...
function contractMultiplier(weeks: number): number {
    if (weeks >= 26) return 1.0;
    if (weeks >= 13) return 0.8;
    return 0.5;
}

function spreadRange(need: string): [number, number] {
    switch (need) {
...
function isAccepted(spread: number, willingness: string): boolean {
    switch (willingness) {
 ...
 */
function getContextualBuyerName(player: Player, allTeams: any[], sellerTeam: Team, findContextualBuyers: unknown): string {
    if (findContextualBuyers && allTeams && allTeams.length > 0) {
        try {
            const buyers = (findContextualBuyers as Function)(player, allTeams, sellerTeam);
            if (buyers.length > 0) {
                return buyers[0].teamName;
            }
        } catch (err) { EngineLogger.capture(err as Error, 'MarketPricer.getContextualBuyer'); }
    }
