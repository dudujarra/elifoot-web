/**
 * PressConference — Coletiva de Imprensa
...
export const JOURNALISTS: readonly Journalist[] = [
...
export interface PressEffect {
    ...
    readonly journalistRelation?: number;
    readonly opponentEffect?: boolean;
}

export interface PressOption {
    readonly id: string;
    readonly text: string;
    readonly effect: PressEffect;
}

export type PressContext = 'crise' | 'invicto' | 'rebaixamento' | 'moral_baixa' | 'genérico';

export interface PressQuestion {
    readonly context: PressContext;
    readonly text: string;
    readonly options: readonly PressOption[];
 ...
 */
export function shouldTriggerPress(
    streak: number,
    currentWeek: number,
    position: number,
    totalTeams: number,
    ...
    if (position >= totalTeams - 1) return true;
    if (currentWeek === 19) return true;
    if (currentWeek >= 36) return true;
    return systemRng() < 0.15;
}
...
export function generateQuestion(
    ...
    const journalist: Journalist = JOURNALISTS[Math.floor(systemRng() * JOURNALISTS.length)];
 ...
 */
export function applyPressEffect(
    team: { squad: Array<{ moral?: number }> } | null,
    board: { confidence: number } | null,
    effect: PressEffect,
): void {
    if (!team) return;
    ...
    if (effect.moral) {
        team.squad.forEach(p => {
            p.moral = Math.max(0, Math.min(100, (p.moral || 50) + (effect.moral || 0)));
        });
    }
