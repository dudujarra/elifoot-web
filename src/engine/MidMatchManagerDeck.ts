...
export interface MidMatchEffect {
    moralDelta?: number;
    energyDelta?: number;
    tacticShift?: string;
}

export interface MidMatchOption {
    label: string;
    effect: MidMatchEffect;
    resultText: string;
}

export interface MidMatchCard {
    id: string;
    minuteRange: [number, number];
    text: string;
    options: MidMatchOption[];
    derby?: boolean;
}

export const MidMatchManagerDeck: MidMatchCard[] = [
    {
        id: 'mid_press_high',
        minuteRange: [15, 30],
 ...
 */
export function shouldTriggerMidMatch(minute: number, alreadyTriggered: Set<number> = new Set()): boolean {
    if (!VALID_TRIGGER_MINUTES.has(minute)) return false;
    if (alreadyTriggered.has(minute)) return false;
    return true;
}

export interface ReactiveCard {
    id: string;
    reactiveType: string;
    text: string;
    options: MidMatchOption[];
...
export const ReactiveCards: ReactiveCard[] = [
    {
 ...
 */
export function getReactiveCard(reactiveType: string, seed: number = 0): ReactiveCard | null {
    const candidates = ReactiveCards.filter((c: ReactiveCard) => c.reactiveType === reactiveType);
 ...
 */
export function getMidMatchCard(minute: number, seed: number = 0): MidMatchCard | null {
    const candidates = MidMatchManagerDeck.filter((c: MidMatchCard) => {
 ...
 */
export function getMidMatchCardDerbyAware(minute: number, isDerby: boolean = false, seed: number = 0): MidMatchCard | null {
    if (!isDerby) return getMidMatchCard(minute, seed);

    const derbyCards = MidMatchManagerDeck.filter((c: MidMatchCard) => {
        if (!c.minuteRange || !Array.isArray(c.minuteRange)) return false;
        const [lo, hi] = c.minuteRange;
