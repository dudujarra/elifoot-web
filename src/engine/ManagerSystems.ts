...
import { Player, Team, Manager, MatchResult } from "./types.js";
...
export const TACTICS: Record<string, TacticConfig> = {
...
export const MATCH_CONDITIONS: MatchCondition[] = [
...
export function rollMatchCondition(): MatchCondition {
...
export function calculateWeeklyFinances(team: Team, weekResults: Record<string, any[]>, teamId: string, engine: Engine): FinanceResult {
    const finance: FinanceResult = { income: 0, expenses: 0, details: [] };
    ...
    const totalWages = team.squad.reduce((sum: number, p: Player) => {
    ...
    if ((team.balance || 0) < 500000 && (team.balance || 0) >= -100000) {
        ...
        if ((team.balance || 0) >= 0 && (team._lowMaintenanceWeeks || 0) > 10 && systemRng() < 0.05) {
    ...
    for (const tId in weekResults) {
        const myMatch = weekResults[tId].find((m: MatchResult) => m.home === teamId);
        if (myMatch) {
...
export function generateTransferOffers(team: Team, currentWeek: number): TransferOffer[] {
...
function getRandomBuyer(): string {
    const clubs = [
...
// MORAL DO ELENCO
// ============================================================
export function calculateSquadMoral(team: Team): number {
    if (!team.squad || team.squad.length === 0) return 50;
    const avg = team.squad.reduce((sum: number, p: Player) => sum + (p.moral || 50), 0) / (team.squad.length || 1);
...
export interface TrainingResult {
    success: boolean;
    msg: string;
    improvements: Array<{ name: string; changes: Array<{ attr: string; old: number; now: number }> }>;
}

export function applyTraining(team: Team, trainingType: string): TrainingResult {
    const training = TRAINING_TYPES.find((t: TrainingType) => t.id === trainingType);
    if (!training) return { success: false, msg: "Tipo de treino inválido.", improvements: [] };

    const improvements: Array<{ name: string; changes: Array<{ attr: string; old: number; now: number }> }> = [];

    team.squad.forEach((player: Player) => {
        if (!player.attributes) return;

        // Apply Morphocycle Load
        const loadResult = Morphocycle.calculateDailyLoad(player, trainingType) as any;
        ...
        // Training Growth
        const deltas = Morphocycle.applyTrainingGrowth(player, trainingType) as any;
        const boosted: Array<{ attr: string; old: number; now: number }> = [];
    ...
    const impText = improvements.slice(0, 5).map((i: any) =>
        `${i.name}: ${i.changes.map((c: any) => `+1 ${c.attr}`).join(', ')}`
    ).join(' | ');
...
export interface TeamTalkResult {
    ...
    msg: string;
    talk: TeamTalk | null;
    modifiers?: { ata: number; def: number };
}

export function applyTeamTalk(team: Team, talkId: string): TeamTalkResult {
    const talk = TEAM_TALKS.find((t: TeamTalk) => t.id === talkId);
