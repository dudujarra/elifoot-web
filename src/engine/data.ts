...
let _FIRST_NAMES: string[] | null = null;
let _LAST_NAMES: string[] | null = null;
let _PLAYERS_BY_TEAM: Record<string, any[]> | null = null;
...
const mapSofifaPosition = (pos: string): string => {
    pos = pos.toUpperCase();
...
export const Data = {
    generatePlayerName(): string {
        ensureNames();
        if (_FIRST_NAMES && _FIRST_NAMES.length > 0 && _LAST_NAMES && _LAST_NAMES.length > 0) {
            return `${rng.pick(_FIRST_NAMES)} ${rng.pick(_LAST_NAMES)}`;
        }
        return `Jogador ${rng.int(1, 9999)}`;
    },

    getRandomRealPlayer(pos: string, tier: number) {
        const pool = (realPlayers as any[]).filter((p: any) => mapSofifaPosition(p.position) === pos);
        if (pool.length > 0) {
     ...
     */
    generatePlayer(position: string, tier: number, options: { forceSuper?: boolean, forceAge?: number | null, _teamBudget?: number, teamBudget?: number, realPlayer?: any } = {}): Player {
        const { forceSuper = false, forceAge = null, _teamBudget = 0, realPlayer = null } = options;

        let age: number, ovr: number, potential: number, name: string, specialtyName: string;
        let isSuper = false;
        ...
        specialtyName = specialty ? specialty.name : "Normal";
        ...
        // Físico
        const heightByPos: Record<string, number[]> = { GOL: [185, 200], DEF: [175, 195], MEI: [168, 185], ATA: [170, 190] };
     ...
     */
    generateSquad(tier: number, teamBudget: number = 0, teamName: string = ""): unknown[] {
        const squad: unknown[] = [];
        ...
        // 11 titulares
        formation.forEach((pos: string, _idx: number) => {
            ...
            if (realData) {
                ...
                const options: { forceSuper?: boolean, forceAge?: number | null, teamBudget?: number } = { teamBudget };
        ...
        benchPositions.forEach((pos: string) => {
            const realData = pickRealPlayer(pos);
            let p;
        ...
        while (realRoster.length > 0 && squad.length < 25) {
            const realData = realRoster.shift() as any;
