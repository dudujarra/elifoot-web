...
export interface SeasonalEventEffect {
    moralDelta?: number;
    energyDelta?: number;
}
...
export const SeasonalBREvents: SeasonalEvent[] = [
    {
 ...
 */
export function getSeasonalEvent(week: number): SeasonalEvent | null {
    if (typeof week !== 'number') return null;
    return SeasonalBREvents.find((e: SeasonalEvent) => e.week === week) || null;
}

/**
 * Lista de semanas que disparam eventos sazonais.
 */
export function getSeasonalTriggerWeeks(): number[] {
    return SeasonalBREvents.map((e: SeasonalEvent) => e.week);
}

/**
 * Conta eventos para validação.
 */
export function getSeasonalEventCount(): number {
