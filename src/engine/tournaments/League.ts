...
export class League extends Tournament {
    ...
    generateRoundRobin(teamIds: string[]) {
        ...
        for (let round = 0; round < rounds; round++) {
            ...
            // Rotate
            rotating.push(rotating.shift() as (string | null));
        }
        return fixtures;
    }
    ...
    advanceWeek(engine: any, week: number) {
        ...
        matches.forEach((m: any) => {
            ...
            // Update standings
            const homeRow = this.standings.find((s: any) => s.teamId === m.home);
            const awayRow = this.standings.find((s: any) => s.teamId === m.away);
        ...
        // Sort standings
        this.standings.sort((a: any, b: any) => b.points - a.points || (b.goalsFor - b.goalsAgainst) - (a.goalsFor - a.goalsAgainst) || b.goalsFor - a.goalsFor);
