 ...
 */
export function getClubState(teamName: string) {
    return (CLUB_STATE_MAP as any)[teamName] || null;
}
...
export class StateChampionship extends Tournament {
    constructor(id: string, name: string, state: string) {
        ...
        this.weekEnd = 16;
        this.startWeek = 1;
        this.metadata = (STATE_CHAMPIONSHIPS as any)[id] || {};
    }

    init(teamIds: string[]) {
        super.init(teamIds);
        this.standings = teamIds.map((id: string) => ({
    ...
    generateRoundRobin(teamIds: string[]) {
    ...
    advanceWeek(engine: Engine, currentWeek: number) {
    ...
    updateStanding(teamId: string, gf: number, ga: number) {
        const s = this.standings.find((x: any) => x.teamId === teamId);
        if (!s) return;
        s.played++;
        s.goalsFor += gf;
        s.goalsAgainst += ga;
    ...
    getStandings() {
        return [...this.standings].sort((a: any, b: any) => {
