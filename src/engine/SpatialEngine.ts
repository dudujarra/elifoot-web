...
export interface SpatialModifiers {
    homeXgMod: number;
    awayXgMod: number;
    spatialData?: {
        homeStretch: number;
...
export class SpatialEngine {
     ...
     */
    extractTeamSpatialProfile(teamSquad: any[]): SpatialProfile | null {
        if (!teamSquad || teamSquad.length === 0) return null;
        
        const titulars = teamSquad.filter((p: Player) => p.isTitular);
        if (titulars.length === 0) return null;

        const sumAttr = (category: string, attr: string) => titulars.reduce((sum: number, p: Player) => sum + ((p.attributes as any)?.[category]?.[attr] || 10), 0) / titulars.length;
     ...
     */
    calculateCentroid(tacticId: string, profile: SpatialProfile | null): number {
     ...
     */
    calculateStretchIndex(tacticId: string, profile: SpatialProfile | null): number {
     ...
     */
    calculatePPDA(tacticId: string, profile: SpatialProfile | null): number {
     ...
     */
    calculateCounterpressingTime(profile: SpatialProfile | null): number {
     ...
     */
    calculateSpatialMatchModifiers(homeTactic: string, homeSquad: any[], awayTactic: string, awaySquad: any[]): SpatialModifiers {
     ...
     */
    getTransientFatigueModifier(player: Player, minute: number): number {
     ...
     */
    getCognitiveModifier(player: Player, minute: number, isDerby: boolean): number {
        if (!player || !player.attributes) return 1.0;
