...
interface ClubEntry {
    ...
    region?: string;
    stadium?: string;
    voices?: Partial<Record<ContextType, string[]>>;
}
...
const typedVoices = ClubVoices as Record<string, ClubEntry>;

/**
 * Retorna voz contextual de um clube. Fallback genérico se clube não mapeado.
 */
export function getClubVoice(clubName: string, contextType: ContextType, seed: number = 0): string {
    if (!clubName) return '';
    const club = typedVoices[clubName];
 ...
 */
export function getClubMeta(clubName: string): ClubMeta | null {
 ...
 */
export function getMappedClubs(): string[] {
    return Object.keys(typedVoices);
}
