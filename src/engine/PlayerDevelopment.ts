...
import { rng } from './rng';
import { calcMarketValue } from './MarketPricer.ts';
import { ATTRIBUTE_CATEGORIES, generateDetailedAttributes, calculateOvrFromAttributes } from './PlayerAttributes.ts';
 ...
 */
export function ensureAttributes(player: Player): any {
    ...
    if (!player.attributes || typeof player.attributes !== 'object' || !player.attributes.technical) {
        ...
        ['attacking', 'technical', 'tactical', 'defending', 'creativity'].forEach((k: string) => delete (player as any)[k]);
    }
    
    return player;
}

const PERSONALITY_GROWTH: Record<string, number> = {
...
export function processPlayerDevelopment(player: Player): DevelopmentChange[] {
    ensureAttributes(player); // BUG-096: guard contra attributes undefined
    const changes: DevelopmentChange[] = [];
    const personalityMod = PERSONALITY_GROWTH[player.personality || 'Normal'] || 1.0;
    ...
    const getRandomAttrPath = () => {
        ...
        const cat = rng.pick(catKeys) as string;
        if (!cat) return null;
        if (cat === 'goalkeeping' && player.position !== 'GOL') return null; // Don't train GK stats for outfield
        const attr = rng.pick((ATTRIBUTE_CATEGORIES as any)[cat]) as string;
        if (!attr) return null;
        return { cat, attr };
    };
    ...
    if (player.age! >= effectiveDecline) {
        ...
        ATTRIBUTE_CATEGORIES.physical.forEach((attr: string) => {
        ...
        ATTRIBUTE_CATEGORIES.technical.forEach((attr: string) => {
        ...
        ATTRIBUTE_CATEGORIES.mental.forEach((attr: string) => {
    ...
    if (player._peakVariance === undefined) {
        ...
        const hash = typeof player.id === 'string'
            ? player.id.split('').reduce((h: number, c: string) => ((h << 5) - h + c.charCodeAt(0)) | 0, 0)
            : (player.id || 0);
 ...
 */
export function ageSquad(squad: any[]): string[] {
    const events: string[] = [];
...
function _recalcOvr(player: Player): void {
