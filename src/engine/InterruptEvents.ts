...
export const INTERRUPT_EVENT_TYPES: Record<string, string> = {
    MEDICAL: 'medical',
    LEGAL: 'legal',
    FAMILIAL: 'familial',
    BOARDROOM: 'boardroom',
...
export interface InterruptEventEffect {
    boss?: number;
    fans?: number;
    reputation?: number;
    money?: number;
    ...
    dignity?: number;
    journalist?: number;
    quit_to_new_club?: boolean;
    wage_x?: number;
}

export interface InterruptEventOption {
    label: string;
    effect: InterruptEventEffect;
}

export interface InterruptEventContext {
    engine?: any;
    team?: Team;
}
...
export interface InterruptEvent {
    ...
    type: string;
    weight: number;
    trigger: (context: InterruptEventContext) => boolean;
    title: string;
    text: string;
    options: InterruptEventOption[];
}
...
export const INTERRUPT_EVENTS: InterruptEvent[] = [
    // === MEDICAL ===
    {
        ...
        type: 'medical',
        weight: 0.15,
        trigger: ({ team }: InterruptEventContext) => !!team && team.squad.some((p: Player) => (p.ovr ?? 50) >= 80 && !p.injury),
    ...
    // === LEGAL ===
    {
        ...
        type: 'legal',
        weight: 0.05,
        trigger: ({ team }: InterruptEventContext) => !!team && team.squad.some((p: Player) => (p.age || 0) > 22),
    ...
    // === FAMILIAL ===
    {
        ...
        type: 'familial',
        weight: 0.08,
        trigger: ({ team }: InterruptEventContext) => !!team && team.squad.some((p: Player) => (p.age || 0) >= 23 && (p.age || 0) <= 32),
        title: 'Paternidade',
    ...
    // === BOARDROOM ===
    {
        ...
        type: 'boardroom',
        weight: 0.12,
        trigger: ({ engine, team }: InterruptEventContext) => {
            if (!engine || !team) return false;
            const standings = engine.getStandings(team.zone, team.division);
            const pos = standings.findIndex((s: any) => s.teamId === team.id) + 1;
            return pos <= 6 && pos >= 4;
        },
    ...
    },
    {
        ...
        type: 'boardroom',
        weight: 0.08,
        trigger: ({ engine }: InterruptEventContext) => !!engine && engine.currentSponsor && engine.managerStats?.streak < -3,
    ...
    // === OPPORTUNITY ===
    {
        ...
        type: 'opportunity',
        weight: 0.04,
        trigger: ({ engine, team }: InterruptEventContext) => !!team && !!engine && (team.division || 4) <= 2 && engine.legacy?.prestige >= 200,
...
export function pickInterruptEvent(engine: any, team: Team): InterruptEvent | null {
    const eligible = INTERRUPT_EVENTS.filter((ev: InterruptEvent) => {
        try {
            return ev.trigger({ engine, team });
        } catch (err) {
            EngineLogger.capture(err as Error, 'InterruptEvents.trigger');
            return false;
        }
    });
    ...
    const totalWeight = eligible.reduce((s: number, e: InterruptEvent) => s + e.weight, 0);
