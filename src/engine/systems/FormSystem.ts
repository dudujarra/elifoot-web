...
export function initForm(player: Player) {
    if (!player.form) {
        player.form = { last5: [], trend: 'normal' };
    }
}

export function updateForm(player: Player, result: number) {
    initForm(player);
    ...
    const avg = player.form!.last5.length > 0
        ? player.form!.last5.reduce((s: number, v: number) => s + v, 0) / player.form!.last5.length
        : 0;
...
export function getFormEmoji(trend: string) {
    if (trend === 'hot') return '[HOT]';
    if (trend === 'cold') return '[COLD]';
    return '';
}

export function getFormModifier(trend: string) {
