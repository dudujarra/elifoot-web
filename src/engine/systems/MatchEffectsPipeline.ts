...
export class MatchEffectsPipeline {
     ...
     */
    static applyEffects(engine: Engine, homeId: unknown, awayId: unknown, tacticData = null) {
        const homeTeam = engine.getTeam(homeId as string | number);
        const awayTeam = engine.getTeam(awayId as string | number);

        const homeSectors = engine.getTeamSectors(homeId as string);
        const awaySectors = engine.getTeamSectors(awayId as string);
        ...
        // Tactic setup
        const homeTactic = (tacticData as any)?.homeTactic || (isManagerHome
            ? engine.currentTactic
            : ((homeTeam as any)?.npcTacticState?.currentTactic || 'normal'));
        const awayTactic = (tacticData as any)?.awayTactic || (isManagerAway
            ? engine.currentTactic
            : ((awayTeam as any)?.npcTacticState?.currentTactic || 'normal'));
        ...
        if (isManagerMatch) {
            ...
            const oldStreak = (engine.managerStats as any)?.tacticStreak || 0;
            const oldLastTactic = (engine.managerStats as any)?.lastTactic;
        ...
        const rawHomeCounter = (TACTIC_COUNTERS as any)[homeTactic]?.[awayTactic] || 1.0;
        const rawAwayCounter = (TACTIC_COUNTERS as any)[awayTactic]?.[homeTactic] || 1.0;
        ...
        if (isManagerMatch) {
            const streak = (engine.managerStats as any)?.streak || 0;
            const difficulty = getDifficulty();
            const rawBoost = calcOpponentBoost(streak as number);
            if ((streak as number) > 0) {
                opponentBoost = rawBoost;
            } else if ((streak as number) < 0) {
        ...
        if (isManagerHome) {
            const bonus = getWinStreakBonus((engine.manager?.teamId as number) || 0);
        ...
        // Bicho (Match Bonus)
        const bichoBuff = getMatchBonusBuff(engine as any);
        if (bichoBuff > 1.0) {
            if (isManagerHome) {
        ...
        const homeClimate = homeTeam?.climateZone || zones[((homeId as number) || 0) % zones.length];
        const awayClimate = awayTeam?.climateZone || zones[((awayId as number) || 0) % zones.length];
