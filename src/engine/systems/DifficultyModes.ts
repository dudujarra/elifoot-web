...
let _memoryFallback: string | null = null; // in-memory fallback for headless/Node/test envs

export function getDifficulty() {
    try {
        const id = localStorage.getItem(STORAGE_KEY);
        if (id && (DIFFICULTY_MODES as any)[id]) return (DIFFICULTY_MODES as any)[id];
    } catch (err) { EngineLogger.capture(err as Error, 'DifficultyModes.getDifficulty'); }
    // Fallback: in-memory state (set by setDifficulty in headless mode)
    if (_memoryFallback && (DIFFICULTY_MODES as any)[_memoryFallback]) {
        return (DIFFICULTY_MODES as any)[_memoryFallback];
    }
    return DIFFICULTY_MODES.normal;
}

export function setDifficulty(modeId: string) {
    if (!(DIFFICULTY_MODES as any)[modeId]) return false;
    _memoryFallback = modeId; // always set in-memory
    try { localStorage.setItem(STORAGE_KEY, modeId); } catch (err) { EngineLogger.capture(err as Error, 'DifficultyModes.setDifficulty'); }
    return true;
}

function _applyDifficultyToValue(rawValue: number, modifierKey: string) {
    const diff = getDifficulty();
    const mult = (diff.modifiers as any)[modifierKey] ?? 1.0;
    return Math.round(rawValue * mult);
}

// SPEC-147: DDA calibrated boost curve (data: deep soak max win streak 18, max loss 11)
export function calcOpponentBoost(streak: number) {
    if (streak <= 0) {
