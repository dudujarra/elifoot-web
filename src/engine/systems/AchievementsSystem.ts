...
export class AchievementsSystem {
    ...
    unlockAchievement(entityId: string | number, achievementId: string, weekOfYear = 0) {
        if (!(ACHIEVEMENTS as any)[achievementId]) return null;
        ...
        const achievement = (ACHIEVEMENTS as any)[achievementId];
    ...
    setProgress(entityId: string | number, achievementId: string, value: number) {
    ...
    getProgress(entityId: string | number, achievementId: string) {
        const key = `${entityId}_${achievementId}`;
        return this.progress.get(key) || 0;
    }

    getAchievements(entityId: string | number) {
        const set = this.unlocked.get(entityId) || new Set();
        return [...set].map((id: string) => ({ id, ...(ACHIEVEMENTS as any)[id], unlocked: true }));
    }

    getPoints(entityId: string | number) {
        return this.points.get(entityId) || 0;
    }

    checkAchievements({ entityId, event, details = {} }: { entityId: string | number, event: string, details?: any }) {
        const unlocked = [];
    ...
    processMilestone(entityId: string | number, type: string, value: string | number) {
        const key = `${type}_${value}`;
        if (!(MILESTONES as any)[key]) return null;
        return (MILESTONES as any)[key];
    }

    unlocked: Map<string | number, Set<string>>;
