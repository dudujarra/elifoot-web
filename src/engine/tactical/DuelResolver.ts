 ...
 */
export function resolveGenericDuel(qualityA: number, qualityB: number, tacticalMod = 0, temperature = 5.0) {
    // Add noise epsilon N(0, 1) approximation — MUST use seeded rng, not Math.random
    const epsilon = (rng() + rng() - 1); 
 ...
 */
export function resolveDribbleDuel(attacker: Record<string, number>, defender: Record<string, number>) {
 ...
 */
export function resolveAerialDuel(attacker: Record<string, number>, defender: Record<string, number>) {
 ...
 */
export function resolvePress(presser: Record<string, number>, ballCarrier: Record<string, number>, blockCompactness = 0) {
 ...
 * @returns {boolean}
 */
export function resolvePassCompletion(passer: Record<string, number>, underPressure: boolean, pressureIntensity: number, distancePenalty = 0) {
