...
export class SpatialGrid {
     ...
     */
    addPlayerToZone(player: Player, team: 'A' | 'B', x: number, y: number) {
     ...
     */
    clearGrid() {
        this.zones.forEach((zone: { teamA: Player[], teamB: Player[] }) => {
            zone.teamA = [];
            zone.teamB = [];
        });
    }
    ...
    computeZoneControl(matchContext = {}) {
        return this.zones.map((zone: { teamA: Player[], teamB: Player[] }, idx: number) => {
            ...
            let qualA = zone.teamA.reduce((sum: number, p: Player) => sum + ((p.effectiveQuality as number) || 10), 0);
            let qualB = zone.teamB.reduce((sum: number, p: Player) => sum + ((p.effectiveQuality as number) || 10), 0);
            ...
            if ((matchContext as any).formationMultiplier && (matchContext as any).formationMultiplier[idx]) {
                rawControl *= (matchContext as any).formationMultiplier[idx];
            }
    ...
    width: number;
    height: number;
    zones: { teamA: Player[], teamB: Player[] }[];
