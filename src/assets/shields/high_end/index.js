import flamengo from './flamengo.png';
import palmeiras from './palmeiras.png';
import sao_paulo from './sao_paulo.png';
import corinthians from './corinthians.png';
import atletico_mg from './atletico_mg.png';
import gremio from './gremio.png';
import internacional from './internacional.png';
import fluminense from './fluminense.png';
import vasco from './vasco.png';
import cruzeiro from './cruzeiro.png';
import athletico_pr from './athletico_pr.png';
import botafogo from './botafogo.png';
import bahia from './bahia.png';
import fortaleza from './fortaleza.png';
import ceara from './ceara.png';
import sport_recife from './sport_recife.png';
import vitoria from './vitoria.png';
import goias from './goias.png';
import coritiba from './coritiba.png';
import santos from './santos.png';

import ponte_preta from './ponte_preta.png';
import guarani from './guarani.png';
import juventude from './juventude.png';
import criciuma from './criciuma.png';
import vila_nova from './vila_nova.png';
import atletico_go from './atletico_go.png';
import america_mg from './america_mg.png';
import chapecoense from './chapecoense.png';
import crb from './crb.png';
import novorizontino from './novorizontino.png';
import mirassol from './mirassol.png';
import avai from './avai.png';
import paysandu from './paysandu.png';
import remo from './remo.png';
import sampaio_correa from './sampaio_correa.png';
import nautico from './nautico.png';
import tombense from './tombense.png';
import ituano from './ituano.png';
import botafogo_sp from './botafogo_sp.png';
import operario_pr from './operario_pr.png';

import figueirense from './figueirense.png';
import csa from './csa.png';
import abc from './abc.png';
import londrina from './londrina.png';
import volta_redonda from './volta_redonda.png';
import amazonas from './amazonas.png';
import brusque from './brusque.png';
import sao_bernardo from './sao_bernardo.png';
import aparecidense from './aparecidense.png';
import ypiranga from './ypiranga.png';
import confianca from './confianca.png';
import botafogo_pb from './botafogo_pb.png';
import floresta from './floresta.png';
import sao_jose_rs from './sao_jose_rs.png';
import altos from './altos.png';
import manaus from './manaus.png';
import ferroviario from './ferroviario.png';
import caxias from './caxias.png';
import athletic_club from './athletic_club.png';
import ferroviaria from './ferroviaria.png';

import santa_cruz from './santa_cruz.png';
import parana_clube from './parana_clube.png';
import america_rn from './america_rn.png';
import campinense from './campinense.png';
import treze from './treze.png';
import caldense from './caldense.png';
import sergipe from './sergipe.png';
import brasiliense from './brasiliense.png';
import bangu from './bangu.png';
import moto_club from './moto_club.png';
import nacional_am from './nacional_am.png';
import rio_branco_es from './rio_branco_es.png';
import anapolis from './anapolis.png';
import nova_iguacu from './nova_iguacu.png';
import cianorte from './cianorte.png';
import inter_de_limeira from './inter_de_limeira.png';
import portuguesa_rj from './portuguesa_rj.png';
import retro from './retro.png';
import iguatu from './iguatu.png';
import sousa from './sousa.png';

// ENG
import manchester_city from './manchester_city.png';
import arsenal from './arsenal.png';
import liverpool from './liverpool.png';
import manchester_united from './manchester_united.png';
import chelsea from './chelsea.png';
import tottenham from './tottenham.png';
import newcastle from './newcastle.png';
import aston_villa from './aston_villa.png';
import west_ham from './west_ham.png';
import brighton from './brighton.png';

// ESP
import real_madrid from './real_madrid.png';
import barcelona from './barcelona.png';
import atletico_de_madrid from './atletico_de_madrid.png';
import sevilla from './sevilla.png';
import real_sociedad from './real_sociedad.png';
import real_betis from './real_betis.png';
import villarreal from './villarreal.png';
import athletic_bilbao from './athletic_bilbao.png';
import valencia from './valencia.png';
import osasuna from './osasuna.png';

export const HIGH_END_SHIELDS = {
    "Flamengo": flamengo,
    "Palmeiras": palmeiras,
    "São Paulo": sao_paulo,
    "Corinthians": corinthians,
    "Atlético-MG": atletico_mg,
    "Grêmio": gremio,
    "Internacional": internacional,
    "Fluminense": fluminense,
    "Vasco da Gama": vasco,
    "Cruzeiro": cruzeiro,
    "Athletico-PR": athletico_pr,
    "Botafogo": botafogo,
    "Bahia": bahia,
    "Fortaleza": fortaleza,
    "Ceará": ceara,
    "Sport Recife": sport_recife,
    "Vitória": vitoria,
    "Goiás": goias,
    "Coritiba": coritiba,
    "Santos": santos,
    
    "Ponte Preta": ponte_preta,
    "Guarani": guarani,
    "Juventude": juventude,
    "Criciúma": criciuma,
    "Vila Nova": vila_nova,
    "Atlético-GO": atletico_go,
    "América-MG": america_mg,
    "Chapecoense": chapecoense,
    "CRB": crb,
    "Novorizontino": novorizontino,
    "Mirassol": mirassol,
    "Avaí": avai,
    "Paysandu": paysandu,
    "Remo": remo,
    "Sampaio Corrêa": sampaio_correa,
    "Náutico": nautico,
    "Tombense": tombense,
    "Ituano": ituano,
    "Botafogo-SP": botafogo_sp,
    "Operário-PR": operario_pr,

    "Figueirense": figueirense,
    "CSA": csa,
    "ABC": abc,
    "Londrina": londrina,
    "Volta Redonda": volta_redonda,
    "Amazonas": amazonas,
    "Brusque": brusque,
    "São Bernardo": sao_bernardo,
    "Aparecidense": aparecidense,
    "Ypiranga": ypiranga,
    "Confiança": confianca,
    "Botafogo-PB": botafogo_pb,
    "Floresta": floresta,
    "São José-RS": sao_jose_rs,
    "Altos": altos,
    "Manaus": manaus,
    "Ferroviário": ferroviario,
    "Caxias": caxias,
    "Athletic Club": athletic_club,
    "Ferroviária": ferroviaria,

    "Santa Cruz": santa_cruz,
    "Paraná Clube": parana_clube,
    "América-RN": america_rn,
    "Campinense": campinense,
    "Treze": treze,
    "Caldense": caldense,
    "Sergipe": sergipe,
    "Brasiliense": brasiliense,
    "Bangu": bangu,
    "Moto Club": moto_club,
    "Nacional-AM": nacional_am,
    "Rio Branco-ES": rio_branco_es,
    "Anápolis": anapolis,
    "Nova Iguaçu": nova_iguacu,
    "Cianorte": cianorte,
    "Inter de Limeira": inter_de_limeira,
    "Portuguesa-RJ": portuguesa_rj,
    "Retrô": retro,
    "Iguatu": iguatu,
    "Sousa": sousa,

    "Manchester City": manchester_city,
    "Arsenal": arsenal,
    "Liverpool": liverpool,
    "Manchester United": manchester_united,
    "Chelsea": chelsea,
    "Tottenham": tottenham,
    "Newcastle": newcastle,
    "Aston Villa": aston_villa,
    "West Ham": west_ham,
    "Brighton": brighton,

    "Real Madrid": real_madrid,
    "Barcelona": barcelona,
    "Atlético de Madrid": atletico_de_madrid,
    "Sevilla": sevilla,
    "Real Sociedad": real_sociedad,
    "Real Betis": real_betis,
    "Villarreal": villarreal,
    "Athletic Bilbao": athletic_bilbao,
    "Valencia": valencia,
    "Osasuna": osasuna
};
